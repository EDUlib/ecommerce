...
from ecommerce.core.url_utils import get_ecommerce_url, get_lms_url, get_course_catalog_api_client
...
class CouponOfferView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if code is not None:
            ...
            if valid_voucher:
                query_results = self.request.site.siteconfiguration.course_catalog_api_client.course_runs.get(
                    q=voucher.offers.first().benefit.range.catalog_query
                )['results']
                benefit = voucher.offers.first().benefit
                courses = []
                products = [products[0] for i in range(20)]
                for product in products:
                    try:
                        course = get_course_seat(product, query_results, benefit, self.request)
                        courses.append(course)
                    except (ConnectionError, SlumberHttpBaseException, Timeout) as e:
                ...
                context.update({
                    ...
                    'discount_value': "%.2f" % (float(courses_paginated[0]['price']) -
                                                float(courses_paginated[0]['new_price'])),
                    'voucher_valid_until': voucher.end_datetime
                })
                return context
            return {
                'error': msg,
